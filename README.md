...
### Dispatch alerts on the MDT

The MDT's call board mirrors the dispatch resource you already run. Six systems are picked up
automatically, with no snippet and nothing to install, because they announce an alert through an
event: **ps-dispatch**, **qb-dispatch** (and its forks), **cd_dispatch**, **qs-dispatch**,
**rcore_dispatch** and **aty_dispatchv2**. Alerts are de-duplicated when a system re-fires one
incident, and rate limited so one noisy script cannot bury the board. Turn the whole thing off, or
one system off, in `configs/mdt.lua` under `Dispatch.Ingest`.

Mirroring is strictly additive. Every event these resources raise an alert on is a net event, which
any client can fire with whatever it likes - true of those resources with or without sd-phone - so a
mirrored call is quarantined on this board: it holds a share of the board of its own and is the
first thing evicted from it, and it can never be filed at priority 1. Whatever a forged alert
claims, it cannot evict, outrank or displace a call your own officers raised. The board a mirrored
call lands on is the one the jobs in its payload name, which is how all six of these systems address
an alert. The one system that has to be relayed across a client (`aty_dispatchv2`) is the exception
and lands on `Dispatch.Ingest.DefaultDomain`, because nothing raises that payload on the server at
all.

Three systems publish alerts through an **export** instead, and an export call cannot be observed
from outside the resource that made it. There is no way to mirror those automatically, so they reach
the board through the `mdtMirrorCall` export, which puts a call through that same quarantine:

```lua
-- The export itself. Every snippet below is one of these calls with the fields filled in.
exports['sd-phone']:mdtMirrorCall({
    code     = '10-90',                 -- radio code, shown as the chip on the call
    type     = 'Bank Robbery',          -- headline
    priority = 1,                       -- 1 critical .. 4 low (3 when omitted)
    location = 'Legion Square',         -- street text, free-form
    coords   = { x = 240.0, y = 220.0, z = 110.0 },
    domain   = 'leo',                   -- 'leo' for the police board, 'ems' for the medical one
    suspect  = 'Male, black hoodie',    -- optional
    weapon   = 'Carbine Rifle',         -- optional
    direction = 'North',                -- optional
})
```

Use `mdtMirrorCall` for anything a dispatch resource hands you, and above all for anything that
reached your server across a client, which is what each snippet below does. There is a second
export, `mdtCreateCall`, taking the same fields plus `ttl`: that one is **trusted**, and the call it
files ranks and evicts exactly like a call an officer raised. Keep it for alerts your own
server-side code decides on by itself, and never point it at a payload a client sent you.

**tk_dispatch** - `exports.tk_dispatch:addCall(data)`. Add the mirror wherever you already raise a
call. The client form of `addCall` resolves the location itself, so send the MDT what you know. This
covers the calls your own scripts raise; tk_dispatch's built-in shooting, speeding and vehicle-theft
detectors never pass through the public export, so they stay off the board:

```lua
local data = {
    title = 'Shooting', code = '10-11', priority = 'Priority 2',
    coords = GetEntityCoords(cache.ped), showLocation = true, jobs = { 'police' },
}
exports.tk_dispatch:addCall(data)
TriggerServerEvent('my-resource:mirrorToMdt', data)   -- see the server half below
```

```lua
-- Server side, in your own resource. A client fires this event, so `data` is whatever that client
-- sent: mdtMirrorCall is what stops a forged one costing your board anything. tk_dispatch's
-- priority is a free-form label, so match on it rather than on a number.
RegisterNetEvent('my-resource:mirrorToMdt', function(data)
    if type(data) ~= 'table' then return end
    local coords = GetEntityCoords(GetPlayerPed(source))
    exports['sd-phone']:mdtMirrorCall({
        code     = data.code,
        type     = data.title,
        priority = data.priority == 'Priority 1' and 1 or data.priority == 'Priority 2' and 2 or 3,
...
**codem-dispatch** - `exports['codem-dispatch']:CustomDispatch(Data)`, client side only. Its
`editable/client_editable.lua` ships unencrypted and is the vendor's designated modification surface,
so one line inside `CustomDispatch` mirrors every alert the resource raises, including its own:

```lua
-- codem-dispatch/editable/client_editable.lua, inside function CustomDispatch(data),
-- immediately before the existing TriggerServerEvent('codem-dispatch:server:SendAlert', ...)
TriggerServerEvent('my-resource:mirrorToMdt', Data)
```

```lua
-- Server side, in your own resource. `Data` already carries the resolved street in `zone`, and it
-- arrived across a client, so it goes on the board through the quarantined export.
RegisterNetEvent('my-resource:mirrorToMdt', function(Data)
    if type(Data) ~= 'table' then return end
    exports['sd-phone']:mdtMirrorCall({
        code     = Data.code,
        type     = Data.header or Data.type,
        location = Data.zone,
        coords   = Data.coords and { x = Data.coords.x, y = Data.coords.y, z = Data.coords.z },
        suspect  = Data.text,
        domain   = 'leo',
    })
end)
...
**fd_dispatch** - the alert-creating exports are inbound only, but the resource publishes a
server-side `newAlert` hook, which is the cleaner mirror. Put this in your own resource, and return
nothing from the callback: a falsey return cancels the alert inside fd_dispatch. Its documentation
prints the wrong resource name on that page, so check that `exports.fd_dispatch:registerHook`
resolves on your build before relying on it.

```lua
exports.fd_dispatch:registerHook('newAlert', function(payload)
    local alert = payload and payload.data
    if not alert then return end

    local ems = false
    for _, group in ipairs(alert.groups or {}) do
        if group == 'ambulance' or group == 'fire' then ems = true end
    end

    local coords = alert.location and alert.location.coords
    exports['sd-phone']:mdtMirrorCall({
        code     = alert.code,
        type     = alert.title,
        priority = alert.isEmergency and 1 or alert.priority,
        location = alert.location and alert.location.street,
        coords   = coords and { x = coords.x, y = coords.y, z = coords.z },
        suspect  = alert.description,
        domain   = ems and 'ems' or 'leo',
    })
end)
```
...
| Dispatch | ps-dispatch, qb-dispatch, cd_dispatch, qs-dispatch, rcore_dispatch, aty_dispatchv2 mirrored onto the MDT call board |
